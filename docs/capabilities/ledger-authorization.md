...
The Rust reference shared submission path now refreshes that plan before signing and only continues when its selected local Ed25519 signer alone can satisfy every current source-account requirement. Transactions requiring additional signer weight or typed conditions fail explicitly and remain for Signing Coordination; this direct-path guard does not collect signatures or claim general multisig completion.
