...
For product review, the Rust reference can derive an immutable `LedgerAuthorizationSnapshot` bound to the prepared transaction hash. The snapshot reports each source account's required weight, weight already satisfied by authorization material on the envelope, additional locally available software-Ed25519 weight, remaining weight after those local capabilities, typed signer availability, and mandatory `extraSigners`. Payment, Trustline and SDEX Offer reviews carry this snapshot directly. This is review evidence only: submission still reloads current ledger authorization and coordinates signing against that fresh plan rather than trusting the snapshot.
