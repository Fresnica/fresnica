...
def sdk_client():
    ...
    if not binary:
        pytest.skip("FRESNICA_PROCESS_BIN is not configured")
...
def test_runtime_prefers_process_binding(tmp_path, monkeypatch, sdk_client):
    monkeypatch.setenv("FRESNICA_PROCESS_BIN", str(sdk_client.binary))
    runtime = Runtime(home=tmp_path)
    assert isinstance(runtime.core_client, FresnicaProcessClient)
